...
export default function MeetingsPage() {
  ...
  const hasEvents = (day: number) => {
    ...
    return (calendarData?.meetings?.[dateKey]?.length ?? 0) > 0;
  };
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: getDaysInMonth(currentDate) }).map((_, index) => {
                ...
                return (
                  <div
                    key={day}
                    className={`h-24 p-1 border border-gray-200 rounded-md cursor-pointer hover:bg-gray-50 ${
                      isTodayDate ? 'bg-blue-50 border-blue-200' : ''
                    } ${selectedDate === getDateKey(day) ? 'ring-2 ring-blue-500' : ''}`}
                  ...
                  >
                    <div className="h-full flex flex-col">
                      ...
                      <div className="flex-1 overflow-hidden">
                        {dayEvents.slice(0, 2).map((meeting, eventIndex) => {
                          ...
                          return (
                            <div
                              key={eventIndex}
                              className={`text-xs p-1 mb-1 rounded truncate ${statusColors.bg} ${statusColors.text}`}
          ...
          {selectedDate && (
            <div className="border-t border-gray-200 p-6">
              ...
              {getEventsForDay(new Date(selectedDate).getDate()).length === 0 ? (
                <p className="text-gray-500">Brak spotkań w tym dniu</p>
              ) : (
                <div className="space-y-3">
                  {getEventsForDay(new Date(selectedDate).getDate()).map((meeting) => {
