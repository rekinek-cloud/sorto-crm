...
export default function KnowledgeStatusPage() {
  ...
  const loadStats = async () => {
    try {
      const res = await fetch('/crm/api/v1/ai-knowledge/stats', {
        headers: {
  ...
  const loadSuggestions = async () => {
    try {
      const res = await fetch('/crm/api/v1/ai-knowledge/suggestions', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
  ...
  const handleQuery = async (question: string) => {
    ...
    try {
      const res = await fetch('/crm/api/v1/ai-knowledge/query', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
