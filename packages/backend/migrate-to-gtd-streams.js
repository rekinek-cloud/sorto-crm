...
async function migrateToGTDStreams() {
  const result = {
    migratedStreams: 0,
    createdStreams: 0,
    errors: []
  ...
  try {
    ...
    for (const stream of existingStreams) {
      try {
        ...
        const errorMsg = `Failed to migrate stream ${stream.name}: ${error.message}`;
    ...
    const createdCount = await ensureDefaultGTDStructure();
    result.createdStreams = createdCount;
    ...
    console.log(`   • Newly created: ${result.createdStreams}`);
    ...
    result.errors.push(`Global migration error: ${error.message}`);
  }
 ...
 */
function determineGTDRole(stream) {
 ...
 */
function determineStreamType(stream, gtdRole) {
  switch (gtdRole) {
 ...
 */
function generateDefaultGTDConfig(gtdRole) {
  const baseConfig = {
    autoRouting: true,
 ...
 */
async function ensureDefaultGTDStructure() {
 ...
 */
function getDefaultIconForRole(role) {
  const icons = {
    [GTDRole.INBOX]: '📥',
    [GTDRole.NEXT_ACTIONS]: '⚡',
    [GTDRole.PROJECTS]: '🎯',
...
migrateToGTDStreams()
  .then((result) => {
    ...
    if (result.migratedStreams > 0 || result.createdStreams > 0) {
      console.log(`✅ Successfully migrated ${result.migratedStreams} streams to GTD`);
      console.log(`✅ Created ${result.createdStreams} new GTD streams`);
    }
