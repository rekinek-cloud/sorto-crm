...
router.get('/time-blocks', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      ...
      meta: {
        total: blocks.length,
        workBlocks: blocks.filter(b => !b.isBreak).length,
        breakBlocks: blocks.filter(b => b.isBreak).length
      }
    });
  } catch (error) {
...
router.post('/time-blocks', authMiddleware, async (req, res) => {
  try {
    ...
    const conflictingBlock = await prisma.energy_time_blocks.findFirst({
      where: {
        userId,
    ...
    if (conflictingBlock) {
      return res.status(409).json({
        success: false,
        error: 'Time block conflicts with existing block',
        conflictWith: conflictingBlock
      });
    }

    const block = await prisma.energy_time_blocks.create({
      data: {
        name,
        startTime,
        endTime,
        ...
        userId,
        organizationId,
        order: await getNextOrder(userId, dayOfWeek)
      }
    });
...
router.put('/time-blocks/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const block = await prisma.energy_time_blocks.findFirst({
      where: { id, userId }
    });
    ...
    const updatedBlock = await prisma.energy_time_blocks.update({
      where: { id },
      data: {
        ...updateData,
        updatedAt: new Date()
...
router.post('/schedule-tasks', authMiddleware, async (req, res) => {
  try {
    ...
    // 1. Pobierz bloki czasowe na dany dzień
    const timeBlocks = await prisma.energy_time_blocks.findMany({
      where: {
        ...
        OR: [
          { dayOfWeek: dayOfWeek },
          { dayOfWeek: null } // Bloki codzienne
        ]
      },
      orderBy: { startTime: 'asc' }
    });

    // 2. Pobierz dostępne zadania
    const availableTasks = await prisma.task.findMany({
      where: {
        ...
        dueDate: {
          lte: new Date(schedulingDate.getTime() + 24 * 60 * 60 * 1000) // Do jutra
        }
      },
      include: {
        context: true
    ...
    // 3. Usuń stare harmonogramy jeśli force reschedule
    if (forceReschedule) {
    ...
    for (const block of timeBlocks) {
      const blockDurationMinutes = calculateDuration(block.startTime, block.endTime);
      let remainingMinutes = blockDurationMinutes;

      // 4a. Sprawdź primary context
      const primaryTasks = availableTasks.filter(task => 
        task.context?.name === block.primaryContext &&
        task.energy === block.energyLevel &&
        (task.estimatedHours || 0.5) * 60 <= remainingMinutes
      );

      let tasksForBlock = primaryTasks;
      let usedContext = block.primaryContext;

      // 4b. FALLBACK SYSTEM - sprawdź alternative contexts
      if (primaryTasks.length === 0 && block.alternativeContexts) {
        const alternatives = Array.isArray(block.alternativeContexts) 
          ? block.alternativeContexts 
          : JSON.parse(block.alternativeContexts as string);

        for (const altContext of alternatives) {
          const altTasks = availableTasks.filter(task => 
            task.context?.name === altContext &&
            (task.estimatedHours || 0.5) * 60 <= remainingMinutes
          );

          if (altTasks.length > 0) {
            tasksForBlock = altTasks;
            usedContext = altContext;
            break;
          }
        }
      }

      // 4c. Przydziel zadania do bloku
        ...
        if (taskMinutes <= remainingMinutes) {
          const scheduledTask = await prisma.scheduled_tasks.create({
            data: {
              ...
              context: usedContext,
              energyRequired: task.energy || block.energyLevel,
              priority: task.priority,
              scheduledDate: schedulingDate,
              status: 'PLANNED',
              wasRescheduled: usedContext !== block.primaryContext,
              rescheduledFrom: usedContext !== block.primaryContext ? block.primaryContext : null,
              rescheduledReason: usedContext !== block.primaryContext ? 'NO_TASKS_IN_CONTEXT' : null,
              userId,
              organizationId
            }
          });
          ...
          const taskIndex = availableTasks.findIndex(t => t.id === task.id);
          if (taskIndex > -1) {
    ...
    // 5. Zadania które nie zostały zaplanowane
    unscheduledTasks.push(...availableTasks);
    ...
    res.json({
      ...
      data: {
        ...
        unscheduledTasks,
        statistics: {
          totalTasks: scheduledTasks.length + unscheduledTasks.length,
          scheduledCount: scheduledTasks.length,
          unscheduledCount: unscheduledTasks.length,
          schedulingRate: scheduledTasks.length / (scheduledTasks.length + unscheduledTasks.length),
          blocksUsed: timeBlocks.filter(b => 
            scheduledTasks.some(st => st.energyTimeBlockId === b.id)
          ).length,
...
router.post('/scheduled-tasks', authMiddleware, async (req, res) => {
  try {
    ...
    // Parse scheduled date to get target date info
    const targetDate = new Date(scheduledDate);
    const dateString = targetDate.toISOString().split('T')[0];
    const dayOfWeek = getDayOfWeek(targetDate);

    // Find suitable time block for this event
    let energyTimeBlockId = null;
    
    if (specificTimeSlot) {
      // Find or create a specific time block for this time slot
      const { date, startTime, endTime } = specificTimeSlot;
      
      // Look for existing block at this time
      const existingBlock = await prisma.energy_time_blocks.findFirst({
        where: {
          userId,
          isActive: true,
          startTime,
          endTime,
          OR: [
            { dayOfWeek: null }, // specific day blocks
            { dayOfWeek: dayOfWeek }, // template blocks
            { workdays: true } // workday templates
          ]
        }
      });

      if (existingBlock) {
        energyTimeBlockId = existingBlock.id;
      } else {
        // Create a new time block for this specific event
        const newBlock = await prisma.energy_time_blocks.create({
          data: {
            name: `Event: ${title}`,
            startTime,
            endTime,
            ...
            alternativeContexts: [],
            isBreak: false,
            dayOfWeek: null, // specific day block
            workdays: false,
            weekends: false,
            holidays: false,
            specificDays: [],
        ...
        energyTimeBlockId = newBlock.id;
      }
    } else {
      // Find the best suitable existing block based on time and energy
      const targetHour = targetDate.getHours();
      const timeString = `${targetHour.toString().padStart(2, '0')}:${targetDate.getMinutes().toString().padStart(2, '0')}`;
      
      const suitableBlock = await prisma.energy_time_blocks.findFirst({
        where: {
          userId,
          isActive: true,
          isBreak: false,
      ...
      energyTimeBlockId = suitableBlock?.id || null;
    }
...
router.get('/daily-schedule/:date', authMiddleware, async (req, res) => {
  try {
    ...
    // Pobierz bloki czasowe dla konkretnej daty
    const dayStart = new Date(schedulingDate);
    dayStart.setHours(0, 0, 0, 0);
    const dayEnd = new Date(schedulingDate);
    dayEnd.setHours(23, 59, 59, 999);
    
    // Get time blocks with proper deduplication (same logic as dashboard)
    const allTimeBlocks = await prisma.energy_time_blocks.findMany({
      where: {
        userId,
        isActive: true,
        OR: [
          // Template blocks for this day of week
          { 
            dayOfWeek: dayOfWeek,
            createdAt: {
              lt: dayStart // Template blocks created before today
            }
          },
          // Specific day blocks created for today
          { 
            dayOfWeek: null,
            createdAt: {
              gte: dayStart,
          ...
          // Fallback for workdays template (same as dashboard)
          { 
            workdays: true,
            dayOfWeek: null, // Only generic workday templates
            createdAt: {
              lt: dayStart
            }
          }
    ...
    // Deduplicate blocks by startTime-endTime combination (keep the newest)
    const uniqueBlocks = new Map();
    allTimeBlocks.forEach(block => {
      const key = `${block.startTime}-${block.endTime}`;
      if (!uniqueBlocks.has(key) || uniqueBlocks.get(key).createdAt < block.createdAt) {
        uniqueBlocks.set(key, block);
      }
    });
    
    const timeBlocks = Array.from(uniqueBlocks.values()).sort((a, b) => 
      a.startTime.localeCompare(b.startTime)
    );

    // Oblicz statystyki
    const allScheduledTasks = timeBlocks.flatMap(block => block.scheduled_tasks);
    const totalPlannedMinutes = allScheduledTasks.reduce((sum, task) => sum + task.estimatedMinutes, 0);
    ...
    res.json({
      ...
      data: {
        ...
        statistics: {
          ...
          totalPlannedMinutes,
          totalWorkMinutes,
          utilizationRate: totalWorkMinutes > 0 ? totalPlannedMinutes / totalWorkMinutes : 0,
          blocksWithTasks: workBlocks.filter(b => b.scheduled_tasks.length > 0).length
        }
      }
    });
  } catch (error) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const timeBlocks = await prisma.energy_time_blocks.findMany({
        where: {
          ...
          OR: [
            { dayOfWeek: dayOfWeek },
            { workdays: true, dayOfWeek: null },
            { dayOfWeek: null, createdAt: { gte: dayStart, lte: dayEnd } }
          ]
        },
        include: {
          scheduled_tasks: {
      ...
      weekData.push({
        ...
        statistics: {
          ...
          totalTasks: allScheduledTasks.length,
          totalPlannedMinutes,
          blocksWithTasks: timeBlocks.filter(b => b.scheduled_tasks.length > 0).length
        }
      });
    }
...
router.delete('/focus-modes/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // Usuń powiązania z time blocks
    await prisma.energy_time_blocks.updateMany({
      where: { focusModeId: id },
      data: { focusModeId: null }
    });
...
router.post('/time-blocks/:id/focus-mode', authMiddleware, async (req, res) => {
  try {
    ...
    const { focusModeId } = req.body;

    const timeBlock = await prisma.energy_time_blocks.findFirst({
    ...
    res.json({
      success: true,
      data: updatedBlock,
      message: 'Focus mode assigned to time block successfully'
    });
  } catch (error) {
...
router.post('/performance-analytics/generate', authMiddleware, async (req, res) => {
  try {
    ...
    // Pobierz dane z EnergyAnalytics w danym okresie
    ...
    // Pobierz zaplanowane zadania w okresie
    const scheduledTasks = await prisma.scheduled_tasks.findMany({
      where: {
        userId,
    ...
    // Analiza focus modes
    const focusModeBlocks = energyAnalytics.filter(ea => ea.energy_time_blocks.focus_modes);
    const focusModeTasksCompleted = focusModeBlocks.reduce((sum, ea) => sum + ea.tasksCompleted, 0);
    const focusModeTasksPlanned = focusModeBlocks.reduce((sum, ea) => sum + ea.tasksPlanned, 0);
    const focusModeEfficiency = focusModeTasksPlanned > 0 ? focusModeTasksCompleted / focusModeTasksPlanned : 0;

    // Analiza kontekstów
    const contextSwitches = energyAnalytics.reduce((sum, ea) => sum + ea.contextSwitches, 0);
    const avgProductivity = energyAnalytics.length > 0 
      ? energyAnalytics.reduce((sum, ea) => sum + (ea.productivityScore || 0), 0) / energyAnalytics.length
      : 0;

    // Oblicz wykorzystanie czasu
    const totalPlannedMinutes = energyAnalytics.reduce((sum, ea) => sum + ea.minutesPlanned, 0);
    const totalActualMinutes = energyAnalytics.reduce((sum, ea) => sum + ea.minutesActual, 0);
    const timeBlockUtilization = totalPlannedMinutes > 0 ? totalActualMinutes / totalPlannedMinutes : 0;

    // Analiza konsekwencji energii
    const energyConsistency = calculateEnergyConsistency(energyAnalytics);

    // Oblicz streak produktywności
    const { currentStreak, longestStreak } = calculateProductivityStreak(energyAnalytics);

    // Oblicz średni poziom zadowolenia
    const satisfactionScores = energyAnalytics.filter(ea => ea.satisfactionScore);
    const avgSatisfaction = satisfactionScores.length > 0
      ? satisfactionScores.reduce((sum, ea) => sum + (ea.satisfactionScore || 0), 0) / satisfactionScores.length
      : null;

    // Wygeneruj sugestie AI
    ...
    // Utwórz metryki wydajności
    const performanceMetrics = await prisma.performance_metrics.create({
      data: {
        startDate: start,
        endDate: end,
        periodType,
        focusModeId: focusModeBlocks[0]?.energy_time_blocks.focus_modes?.id || null,
        focusModeEfficiency,
        focusModeProductivity: avgProductivity,
        primaryContext: getMostUsedContext(energyAnalytics),
        alternativeContexts: getAlternativeContexts(energyAnalytics),
        contextSwitchCount: contextSwitches,
        contextEfficiency: avgProductivity,
        energyLevel: getMostCommonEnergyLevel(energyAnalytics),
        energyConsistency,
        energyOptimalTimes: getOptimalEnergyTimes(energyAnalytics),
        totalTasks,
        completedTasks,
        overdueTasks,
        completionRate,
        ...
        timeBlockUtilization,
        productiveStreakDays: currentStreak,
        currentStreak,
        longestStreak,
        satisfactionTrend: avgSatisfaction,
        burnoutRisk,
        suggestions,
        organizationId,
        userId
...
router.post('/energy-feedback', authMiddleware, async (req, res) => {
  try {
    ...
    // Pobierz blok czasowy i zaplanowane zadania
    const timeBlock = await prisma.energy_time_blocks.findFirst({
      where: { id: energyTimeBlockId, userId },
      include: {
        scheduled_tasks: {
          where: {
            scheduledDate: {
    ...
    const plannedTasks = timeBlock.scheduled_tasks.length;
    const plannedMinutes = timeBlock.scheduled_tasks.reduce((sum, task) => sum + task.estimatedMinutes, 0);
    const productivityScore = tasksCompleted / Math.max(plannedTasks, 1);

    // Zapisz analitykę
    const analytics = await prisma.energy_analytics.create({
      data: {
        energyTimeBlockId,
        plannedEnergy: timeBlock.energyLevel,
        actualEnergy,
        energyScore,
        ...
        minutesActual,
        productivityScore,
        contextsPlanned: [timeBlock.primaryContext],
        satisfactionScore,
        notes,
        distractions,
        userId,
    ...
    // Aktualizuj wzorce energii (Machine Learning)
    await updateEnergyPatterns(userId, timeBlock, analytics);

    res.status(201).json({
      success: true,
      data: analytics,
      message: 'Energy feedback recorded successfully'
    });
  } catch (error) {
...
async function prioritizeTasksForDay(tasks: any[], preferences: any, date: string) {
  ...
  return tasks.sort((a, b) => {
    ...
    const scoreA = (priorityWeights[a.priority] || 2);
    const scoreB = (priorityWeights[b.priority] || 2);
...
// Helper functions dla Dashboard Integration
function getDayOfWeek(date: Date): string {
  const days = ['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'];
  return days[date.getDay()];
}
...
function getStartOfWeek(date: Date): Date {
  ...
  const diff = result.getDate() - day + (day === 0 ? -6 : 1); // Poniedziałek jako pierwszy dzień
...
async function findNextAvailableTimeBlock(userId: string) {
  const today = new Date();
  const blocks = await prisma.energy_time_blocks.findMany({
    where: {
      userId,
  ...
  // Znajdź blok z najmniejszą liczbą zadań
  return blocks.reduce((best, current) => {
    const currentTasksCount = current.scheduled_tasks?.length || 0;
    const bestTasksCount = best?.scheduled_tasks?.length || 0;
    return currentTasksCount < bestTasksCount ? current : best;
  }, blocks[0] || null);
}

function generateWeeklyRecommendations(dailyStats: any[], productivity: any, bestTimeSlots: any[]): string[] {
  const recommendations = [];

  // Analiza wydajności
  if (productivity.thisWeek > 0.8) {
    recommendations.push('🔥 Świetny tydzień! Kontynuuj obecny rytm pracy');
  } else if (productivity.thisWeek < 0.4) {
    recommendations.push('📈 Zastanów się nad optymalizacją planowania zadań');
  }
  ...
  if (bestDay.completionRate - worstDay.completionRate > 30) {
    recommendations.push(`🎯 ${bestDay.day} to Twój najlepszy dzień - zaplanuj trudniejsze zadania`);
    recommendations.push(`⚠️ ${worstDay.day} wymaga uwagi - rozważ lżejszy plan`);
  }

  // Analiza najlepszych czasów
  if (bestTimeSlots.length > 0) {
    const topSlot = bestTimeSlots[0];
    recommendations.push(`⏰ ${topSlot.time} w ${topSlot.day} to Twój peak time - zarezerwuj na najważniejsze zadania`);
  }

  // Zadania przeterminowane
  if (productivity.overdueTasks > 0) {
    recommendations.push(`🚨 ${productivity.overdueTasks} zadań przeterminowanych - rozważ reorganizację priorytetów`);
  }
