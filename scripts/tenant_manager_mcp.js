  ...
  async ({ tenant_id, include_performance = true, check_quotas = true }) => {
    try {
      ...
      const report = {
        ...
        performance: include_performance ? usage.performance_metrics : undefined,
        scaling_recommendations: scaling.recommendations,
        auto_scale_triggered: scaling.auto_scale_triggered
      };
      ...
      return {
        content: [{
          type: 'text',
          text: `Resource monitoring failed: ${error.message}`
        }]
      };
    }
  }
...
// Tool: Verify tenant isolation
server.tool(
  'verify_tenant_isolation',
  'Verify complete data isolation and check for cross-tenant data leaks',
  {
    tenant_id: z.string().describe('Tenant ID to verify isolation for'),
    detailed_report: z.boolean().optional().describe('Generate detailed isolation report')
  },
  async ({ tenant_id, detailed_report = false }) => {
    try {
      ...
      const report = {
        ...
        checks_passed: Object.values(isolation.checks).filter(check => check.passed).length,
        checks_failed: Object.values(isolation.checks).filter(check => !check.passed).length,
        checks: detailed_report ? isolation.checks : undefined,
        data_leaks_detected: isolation.data_leaks.length,
        cross_queries_detected: isolation.cross_queries.length,
        recommendations: isolation.recommendations,
        compliance_status: isolation.status === 'verified' ? 'COMPLIANT' : 'NON_COMPLIANT'
      };
      ...
      return {
        content: [{
          type: 'text',
          text: `Isolation verification failed: ${error.message}`
        }]
      };
    }
  }
...
// Tool: List all tenants
server.tool(
  'list_tenants',
  'List all tenants with their current status and resource usage',
  {
    include_usage: z.boolean().optional().describe('Include current resource usage data'),
    plan_filter: z.enum(['starter', 'professional', 'enterprise']).optional().describe('Filter by subscription plan')
  },
  async ({ include_usage = false, plan_filter }) => {
    try {
      const tenants = Array.from(tenantsDB.values())
        .filter(tenant => !plan_filter || tenant.plan === plan_filter)
        .map(tenant => ({
          id: tenant.id,
          name: tenant.name,
          subdomain: tenant.subdomain,
          ...
          user_count: tenant.current_usage.users,
          storage_gb: tenant.current_usage.storage_gb,
          billing_status: tenant.billing.status,
          features: tenant.features
        }));
      ...
      return {
        content: [{
          type: 'text',
          text: `Tenant listing failed: ${error.message}`
        }]
      };
    }
  }
...
// Tool: Billing management
server.tool(
  'manage_billing',
  'Manage tenant billing, subscriptions, and payment methods',
  {
    tenant_id: z.string().describe('Tenant ID for billing management'),
    action: z.enum(['create_customer', 'create_subscription', 'cancel_subscription', 'update_payment']).describe('Billing action to perform'),
    plan_id: z.string().optional().describe('Plan ID for subscription creation'),
    customer_email: z.string().email().optional().describe('Customer email for customer creation'),
    customer_name: z.string().optional().describe('Customer name for customer creation')
  },
  async ({ tenant_id, action, plan_id, customer_email, customer_name }) => {
    try {
      const tenant = tenantsDB.get(tenant_id);
      if (!tenant) {
      ...
      switch (action) {
        case 'create_customer':
          if (!customer_email || !customer_name) {
            throw new Error('Customer email and name are required for customer creation');
          }
          const customer = await createStripeCustomer(tenant, { email: customer_email, name: customer_name });
          result = {
        ...
        case 'create_subscription':
          ...
          const subscription = await createSubscription(tenant, plan_id, 'pm_test_payment_method');
          result = {
      ...
      return {
        content: [{
          type: 'text',
          text: `Billing management failed: ${error.message}`
        }]
      };
    }
  }
...
// Resource: Tenant configurations
server.resource(
  'tenant_configurations',
  'Access tenant configuration templates and plans',
  async () => {
    const configurations = {
      plans: TENANT_CONFIG.plans,
      isolation_levels: TENANT_CONFIG.isolation_levels,
      templates: {
        basic_tenant: {
          plan: 'starter',
          isolation_level: 'basic',
          features: ['basic_crm', 'basic_gtd']
        },
        enterprise_tenant: {
          plan: 'enterprise',
...
async function main() {
  ...
  console.error('CRM-GTD Tenant Manager MCP Server running on stdio');
}
